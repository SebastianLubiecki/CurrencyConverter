...
import ConectionWithAPI.REST_CHF;
...
public class ComparatorTest {
    ...
    @Test
    public void chfTest() throws IOException {
       ...
       // when(rest_chf.getCurrencyNow()).thenReturn(new Currency("a", "Frank", "chf", listNow));
        when(REST_NBP.getAskBidRequest(rest_chf.getCurrencyNow())).thenReturn(arrayNow);
        //when(rest_chf.getCurrencyPast()).thenReturn(new Currency("a", "Frank", "chf", listPast));
        when(REST_NBP.getAskBidRequest(rest_chf.getCurrencyNow())).thenReturn(arrayPast);

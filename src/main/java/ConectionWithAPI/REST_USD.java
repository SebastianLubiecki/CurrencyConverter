...
public class REST_USD implements REST_NBP {
    ...
    public Currency getCurrency() throws IOException {
        Gson gson = new Gson();
        Currency currency = gson.fromJson(String.valueOf(REST_NBP.jsonObject("http://api.nbp.pl/api/exchangerates/rates/a/usd/")), Currency.class);
    ...
    public String toString() {
        try {
            return "REST_USD: " + getCurrency().getRates();
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
